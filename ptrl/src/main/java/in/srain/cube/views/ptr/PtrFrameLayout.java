...
public class PtrFrameLayout extends ViewGroup {
    ...
    private boolean mOnlyShowHeaderOrFooter = true;
    ...
    public PtrFrameLayout(Context context, AttributeSet attrs, int defStyle) {
        ...
        if (arr != null) {
            ...
            mOnlyShowHeaderOrFooter = arr.getBoolean(R.styleable.PtrFrameLayout_ptr_only_show_headerorfooter, mOnlyShowHeaderOrFooter);
...
//        mPagingTouchSlop = conf.getScaledTouchSlop() * 2;
    ...
    public boolean isFooterWorking() {
        if (isRefreshing()) {
    ...
    public boolean ismOnlyShowHeaderOrFooter() {
        return mOnlyShowHeaderOrFooter;
    }

    public void setmOnlyShowHeaderOrFooter(boolean mOnlyShowHeaderOrFooter) {
        this.mOnlyShowHeaderOrFooter = mOnlyShowHeaderOrFooter;
    }

    public boolean isHeaderWorking() {
        if (isRefreshing()) {
            if (mPtrIndicator.isHeader()) {
                return true;
            }
    ...
    public boolean dispatchTouchEvent(MotionEvent e) {
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                //如果Y轴的移动距离不够，则不执行下拉刷新的任何逻辑
                if (totalY < mPagingTouchSlop) {
                    return true;
                }
    ...
    /**
     * 修改为该方法，不需要再使用postDelayed的方式，内部已封装
     */
    public void autoRefresh(int delayMinTime) {
        if (delayMinTime <= 0) {
